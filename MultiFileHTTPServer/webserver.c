...
char * getResponseFromURI(char *uri){
    ...
    if(buffer){
        char content_type[1024];strcpy(content_type,detectContentType(file_path));
...
int main(){
    ...
    if (sockfd == -1) {
        perror(RED "webserver (socket)");
        return 1;
    }
    printf(GRN "socket created successfully\n");
    ...
    if (bind(sockfd, (struct sockaddr *)&host_addr, host_addrlen) != 0) {
        perror(RED "webserver (bind)");
        return 1;
    }
    printf(GRN "socket successfully bound to address\n");
    ...
    printf(YEL "server listening for connections\n");
    ...
    for (;;) {
        ...
        if (newsockfd < 0) {
            perror(RED "webserver (accept)");
            continue;
        }
        printf(GRN "connection accepted\n");

        // Get client address
        int sockn = getsockname(newsockfd, (struct sockaddr *)&client_addr,
                                (socklen_t *)&client_addrlen);
        if (sockn < 0) {
            perror(RED "webserver (getsockname)");
            continue;
        }
